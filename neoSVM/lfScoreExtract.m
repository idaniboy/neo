...
for nthDir=7:7;  
    ...
    for nthMeth=1:length(features);
        ...
        for i = 1:nIClips;
            ...
            IlfS(i) = lfScoreGet(data,features{nthMeth},numCh,freq,clipLoc);
        end

        % Scores for interictal clips
        for i = 1:nIIClips;
            [data,~,freq,~]=sParLoad_oldV(strcat(clipLoc,filesep,clipDirNames{nthDir},'_interictal_segment_',num2str(i),'.mat'));
            IIlfS(i) = lfScoreGet(data,features{nthMeth},numCh,freq,clipLoc);
        end
        ...
        figure;plot(IIIlfS);title(strcat(clipDirNames{nthDir},features(nthMeth)));axis square; set(gcf,'color','w');drawnow;
...
% train svm model
SVMModel = fitcsvm(X,labels,'ClassNames',[0 1],'KernelFunction','rbf');
